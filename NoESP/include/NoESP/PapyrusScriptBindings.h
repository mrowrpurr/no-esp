...
namespace NoESP::PapyrusScriptBindings {
    ...
    template <typename T>
    std::vector<T> GetValuesAsArray(const std::string& text,
                                    const std::function<T(const std::string&)>& itemTransformer) {
        ...
        if (text.starts_with('[') && text.ends_with(']')) {
            ...
            while (true) {
                ...
                if (std::regex_search(textBody, matches, getFirstItemWithQuotes)) {
                    ...
                    textBody = textBody.substr(matches.position() + matches.length() +
                                               1);  // Start right after the regex match
    ...
    std::vector<RE::TESForm*> LookupForms(const std::string& formAsText, bool returnOne = false) {
        ...
        static auto formIdWithPluginNamePattern =
            std::regex(R"(^\s*0x([abcdefABCDEF0123456789]+)\|(.*)\s*$)");
        ...
        if (std::regex_search(formAsText, matches, formIdWithPluginNamePattern)) {
            try {
                ...
                _Log_("Problem looking up form '{}'", formAsText);
            }
    ...
    void AutoFillProperties(const RE::BSTSmartPointer<RE::BSScript::Object>& object,
                            FormPropertyMap& manuallyConfiguredProperties) {
        ...
        for (uint32_t i = 0; i < typeInfo->propertyCount; i++) {
            ...
            if (manuallyConfiguredProperties.contains(
                    Utilities::ToLowerCase(propertyName.c_str()))) {
                _Log_("Not using autofill for property {} because it is manually configured",
                      propertyName.c_str());
                continue;
            }
            ...
            if (rawType == TypeInfo::RawType::kArraysEnd || (rawTypeId >= 0 && rawTypeId <= 15)) {
                _Log_("Property is primitive type, cannot be auto filled {}", propertyName.c_str());
                return;
            }
            ...
            if (propertyVariable->IsObject()) {
                ...
                if (form) {
                    RE::VMHandle handle =
                        handlePolicy->GetHandleForObject(form->GetFormType(), form);
                    ...
                    _Log_(
                        "Autofill only supports object properties, no primitives (int, string, "
                        "etc) right now...");
                }
            }
        }
    ...
    void SetProperties(const std::string& scriptName,
        ...
        for (const auto& [propertyName, propertyValue] : propertyMap) {
            try {
                ...
                auto propertyType =
                    propertyTypeCache.GetOrLookupScriptPropertyType(scriptName, propertyName);
                    ...
                    if (property) {
                        ...
                        switch (rawType) {
                            ...
                            case TypeInfo::RawType::kBool:
                                property->SetBool(Utilities::ToLowerCase(
                                                      propertyValue.PropertyValueText) == "true");
                                break;
                            case TypeInfo::RawType::kStringArray: {
                                auto values = GetValuesAsArray<std::string>(
                                    propertyValue.PropertyValueText,
                                    [](const auto& text) { return text; });
                                RE::BSTSmartPointer<RE::BSScript::Array> papyrusArray;
                                vm->CreateArray(TypeInfo{TypeInfo::RawType::kString}, values.size(),
                                                papyrusArray);
                            ...
                            case TypeInfo::RawType::kIntArray: {
                                auto values = GetValuesAsArray<int>(
                                    propertyValue.PropertyValueText,
                                    [](const auto& text) { return std::stoi(text); });
                                RE::BSTSmartPointer<RE::BSScript::Array> papyrusArray;
                                vm->CreateArray(TypeInfo{TypeInfo::RawType::kInt}, values.size(),
                                                papyrusArray);
                                for (int i = 0; i < values.size(); i++) {
                                    papyrusArray->data()[i].SetSInt(values[i]);
                                }
                                property->SetArray(papyrusArray);
                                break;
                            }
                            case TypeInfo::RawType::kFloatArray: {
                                auto values = GetValuesAsArray<float>(
                                    propertyValue.PropertyValueText,
                                    [](const auto& text) { return std::stof(text); });
                                RE::BSTSmartPointer<RE::BSScript::Array> papyrusArray;
                                vm->CreateArray(TypeInfo{TypeInfo::RawType::kFloat}, values.size(),
                                                papyrusArray);
                                for (int i = 0; i < values.size(); i++) {
                                    papyrusArray->data()[i].SetFloat(values[i]);
                                }
                                property->SetArray(papyrusArray);
                                break;
                            }
                            case TypeInfo::RawType::kBoolArray: {
                                auto values = GetValuesAsArray<bool>(
                                    propertyValue.PropertyValueText, [](const auto& text) {
                                        return Utilities::ToLowerCase(text) == "true";
                                    });
                                RE::BSTSmartPointer<RE::BSScript::Array> papyrusArray;
                                vm->CreateArray(TypeInfo{TypeInfo::RawType::kBool}, values.size(),
                                                papyrusArray);
                                for (int i = 0; i < values.size(); i++) {
                                    papyrusArray->data()[i].SetBool(values[i]);
                                }
                                property->SetArray(papyrusArray);
                                break;
                            }
                            default: {
                                if (property->IsArray()) {
                                    ...
                                    GetValuesAsArray<bool>(propertyValue.PropertyValueText,
                                                           [&values](const auto& text) {
                                    ...
                                    RE::BSTSmartPointer<RE::BSScript::Array> papyrusArray;
                                    vm->CreateArray(TypeInfo{TypeInfo::RawType::kObject},
                                                    values.size(), papyrusArray);
                                    int i = 0;
                                    for (auto* form : values) {
                                        if (form) {
                                            ...
                                            if (typeName.empty()) {
                                                _Log_("Could not get a type name for property {}",
                                                      propertyName);
                                            } else {
                                                RE::BSTSmartPointer<RE::BSScript::Object> objectPtr;
                                                vm->CreateObject(typeName, objectPtr);
                                    ...
                                    auto forms = LookupForms(propertyValue.PropertyValueText,
                                                             true);  // Only return one
                                    if (!forms.empty()) {
                                        auto* form = forms[0];
                                        if (form) {
                                            auto typeName = propertyType->PropertyScriptName;
                        ...
                        _Log_("Property (Form?) could not be found: {}",
                              propertyValue.PropertyValueText);
                    }
                }
            } catch (...) {
                _Log_("Error setting property {}", propertyName.c_str());
            }
        }
    }

    void BindToForm(std::string scriptName, const RE::TESForm& form,
                    FormPropertyMap& propertiesToSet, bool addOnce = false) {
        ...
        try {
            ...
            auto* handlePolicy = vm->GetObjectHandlePolicy();
            RE::VMHandle handle =
                handlePolicy->GetHandleForObject(form.GetFormType(), (RE::TESForm*)&form);
            if (handle) {
                // If there is already a script with the same name attached to this object, don't
                // bind a new one
                ...
                if (addOnce) {
                    if (vm->attachedScripts.contains(handle)) {
                        for (auto& attachedScript : vm->attachedScripts.find(handle)->second) {
                            if (attachedScript->GetTypeInfo()->GetName() ==
                                caseInsensitiveScriptName) {
                                _Log_("{} already attached to 0x{:x}, skipping", scriptName,
                                      form.formID);
                ...
                try {
                    ...
                    _Log_("Failed to bind object to handle for '{}'", scriptName);
                    return;
                }
                ...
                if (ref) {
                    ...
                    _Log_("Bound script '{}' to reference '{}' 0x{:x} (base '{}' 0x{:x})!",
                          scriptName, form.GetName(), form.formID, baseForm->GetName(),
                          baseForm->formID);
                } else {
                    _Log_("Bound script '{}' to form '{}' 0x{:x}!", scriptName, form.GetName(),
                          form.formID);
                }
            } else {
                _Log_("Error getting handle for script {} to reference", scriptName);
            }
        } catch (...) {
            _Log_("Error binding script {} to reference", scriptName);
        }
    }

    void BindToFormPointer(std::string scriptName, RE::TESForm* form,
                           FormPropertyMap& propertiesToSet, bool addOnce = false) {
        if (!form) return;
    ...
    void BindToEditorId(const std::string& scriptName, const std::string& editorId,
                        FormPropertyMap& propertiesToSet, bool addOnce = false) {
        auto* form = RE::TESForm::LookupByEditorID(editorId);
        if (form) {
            BindToFormPointer(scriptName, form, propertiesToSet, addOnce);
        } else {
            _Log_("Could not find Form via Editor ID: '{}' for script '{}'", editorId, scriptName);
        }
    }

    void BindToFormId(const std::string& scriptName, RE::FormID formId,
                      FormPropertyMap& propertiesToSet, const std::string optionalPluginFile = "",
                      bool addOnce = false) {
        if (optionalPluginFile.empty()) {
            ...
            if (form) {
                ...
                _Log_("Could not find Form via Form ID: '{}' for script '{}'", formId, scriptName);
            }
        } else {
            ...
            if (dataHandler->GetModIndex(optionalPluginFile) != 255) {
                ...
                if (form) {
                    ...
                    _Log_("Could not find Form via Form ID: '{}' in plugin '{}' for script '{}'",
                          formId, optionalPluginFile, scriptName);
                }
            } else {
                _Log_("Could not find plugin '{}' for script '{}'", optionalPluginFile, scriptName);
            }
        }
    }
}
